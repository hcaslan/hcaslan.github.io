...
import { Box, Button, Card, CardContent, CardMedia, Dialog, DialogActions, DialogContent, DialogTitle, IconButton, Tooltip, Typography } from '@mui/material';
...
const ProjectCard = (props: IProject) => {
    ...
    <Tooltip title={props.githubButton ? 'GitHub' : ''} disableHoverListener={!props.githubButton}>
        <IconButton
            aria-label="GitHub"
    ...
    <Tooltip title={props.websiteButton ? 'Website' : ''} disableHoverListener={!props.websiteButton}>
        <IconButton
            aria-label="Website"
    ...
    <Tooltip title={props.descriptionButton ? 'Description' : ''} disableHoverListener={!props.descriptionButton}>
        <IconButton
        ...
        >
            ...
            sx={{
                ...
                <DialogTitle color="paletteThirdColour.main" >{selectedProject?.name}</DialogTitle>
                <DialogContent
                    sx={{
                        maxHeight: '60vh',
                        overflowY: 'auto',
                ...
                >
                    <Typography color="paletteThirdColour.main">{selectedProject?.description}</Typography>
                    <div style={{ display: "flex", gap: "10px", marginTop: "10px" }}>
                        <Typography>{selectedProject?.details}</Typography>
                    </div>
                    <div style={{ display: "flex", flexDirection: "column", gap: "10px", marginTop: "20px" }}>
                        {selectedProject?.images?.map((image: string, index: number) => (
                            <img key={index} src={image} alt="Project screenshot" style={{ width: "100%", height: "auto" }} />
                        ))}
                    </div>
                </DialogContent>
                <DialogActions>
